...
public class MessageReadStatusService {
    ...
    /**
     * 클라이언트에서 메시지를 읽으면 RedisB에 기록만 해놓는다.
     * MongoDB는 즉시 업데이트하지 않는다.
     */
    ...
    public void flushPendingStatuses() {
        try {
            ...
            for (Object midObj : pending) {
                ...
                // Redis Set에서 메시지를 읽은 userId 목록
                Set<Object> userIds = redis.opsForSet().members(readKey);
                if (userIds == null || userIds.isEmpty()) continue;

                List<String> users = userIds.stream().map(Object::toString).toList();

                Query query = new Query(
                        Criteria.where("_id").is(messageId)
                );

                // readers 배열에 없으면 addToSet
                List<Document> readerDocs =
                        users.stream()
